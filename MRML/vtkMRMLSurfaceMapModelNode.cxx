...
Program:   3D Slicer
Module:    $RCSfile: vtkMRMLSurfaceMapModelNode.cxx,v $
...
#include "vtkMRMLSurfaceMapModelStorageNode.h"
...
//----------------------------------------------------------------------------
vtkMRMLNodeNewMacro(vtkMRMLSurfaceMapModelNode);

//----------------------------------------------------------------------------
vtkMRMLSurfaceMapModelNode::vtkMRMLSurfaceMapModelNode()
{
  this->PolyData = NULL;
}
...
//----------------------------------------------------------------------------
void vtkMRMLSurfaceMapModelNode::Copy(vtkMRMLNode *anode)
{
  int disabledModify = this->StartModify();
  this->Superclass::Copy(anode);
  vtkMRMLSurfaceMapModelNode* modelNode = vtkMRMLSurfaceMapModelNode::SafeDownCast(anode);
...
//---------------------------------------------------------------------------
void vtkMRMLSurfaceMapModelNode::ProcessMRMLEvents ( vtkObject *caller,
                                           unsigned long event,
                                           void *callData )
{
  ...
  if (this->PolyData == vtkPolyData::SafeDownCast(caller) &&
    ...
    {
    this->InvokeEvent(vtkMRMLSurfaceMapModelNode::PolyDataModifiedEvent, NULL);
    }
  this->Superclass::ProcessMRMLEvents(caller, event, callData);
}

//----------------------------------------------------------------------------
vtkMRMLSurfaceMapModelDisplayNode* vtkMRMLSurfaceMapModelNode::GetModelDisplayNode()
{
  return vtkMRMLSurfaceMapModelDisplayNode::SafeDownCast(this->GetDisplayNode());
}

//----------------------------------------------------------------------------
void vtkMRMLSurfaceMapModelNode::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void vtkMRMLSurfaceMapModelNode::SetAndObservePolyData(vtkPolyData *polyData)
{
  ...
  this->Modified();
  this->InvokeEvent( vtkMRMLSurfaceMapModelNode::PolyDataModifiedEvent , this);
}

//---------------------------------------------------------------------------
void vtkMRMLSurfaceMapModelNode::AddPointScalars(vtkDataArray *array)
{
  this->AddScalars(array, vtkAssignAttribute::POINT_DATA);
}

//---------------------------------------------------------------------------
void vtkMRMLSurfaceMapModelNode::AddCellScalars(vtkDataArray *array)
{
  this->AddScalars(array, vtkAssignAttribute::CELL_DATA);
}

//---------------------------------------------------------------------------
void vtkMRMLSurfaceMapModelNode::AddScalars(vtkDataArray *array, int location)
{
  if (array == NULL)
    {
    return;
...
//---------------------------------------------------------------------------
void vtkMRMLSurfaceMapModelNode::RemoveScalars(const char *scalarName)
{
  if (scalarName == NULL)
    {
...
//---------------------------------------------------------------------------
const char * vtkMRMLSurfaceMapModelNode::GetActivePointScalarName(int type)
{
  if (this->PolyData == NULL ||
...
//---------------------------------------------------------------------------
const char * vtkMRMLSurfaceMapModelNode::GetActiveCellScalarName(int type)
{
  if (this->PolyData == NULL ||
...
//---------------------------------------------------------------------------
bool vtkMRMLSurfaceMapModelNode::HasPointScalarName(const char* scalarName)
{
  if (this->PolyData == NULL ||
      this->PolyData->GetPointData() == NULL)
    {
...
//---------------------------------------------------------------------------
bool vtkMRMLSurfaceMapModelNode::HasCellScalarName(const char* scalarName)
{
  if (this->PolyData == NULL ||
      this->PolyData->GetCellData() == NULL)
    {
...
//---------------------------------------------------------------------------
int vtkMRMLSurfaceMapModelNode::GetAttributeTypeFromString(const char* typeName)
{
...
//---------------------------------------------------------------------------
int vtkMRMLSurfaceMapModelNode::SetActivePointScalars(const char *scalarName, int attributeType)
{
  if (this->PolyData == NULL ||
      this->PolyData->GetPointData() == NULL)
    {
...
//---------------------------------------------------------------------------
int vtkMRMLSurfaceMapModelNode::SetActiveCellScalars(const char *scalarName, int attributeType)
{
  if (this->PolyData == NULL ||
      this->PolyData->GetCellData() == NULL)
    {
...
//---------------------------------------------------------------------------
int vtkMRMLSurfaceMapModelNode::CompositeScalars(const char* backgroundName, const char* overlayName,
...
//---------------------------------------------------------------------------
bool vtkMRMLSurfaceMapModelNode::CanApplyNonLinearTransforms()const
{
  return true;
}

//---------------------------------------------------------------------------
void vtkMRMLSurfaceMapModelNode::ApplyTransform(vtkAbstractTransform* transform)
{
...
//---------------------------------------------------------------------------
void vtkMRMLSurfaceMapModelNode::GetRASBounds(double bounds[6])
{
...
//---------------------------------------------------------------------------
vtkMRMLStorageNode* vtkMRMLSurfaceMapModelNode::CreateDefaultStorageNode()
{
  return vtkMRMLStorageNode::SafeDownCast(vtkMRMLSurfaceMapModelStorageNode::New());
}

//---------------------------------------------------------------------------
void vtkMRMLSurfaceMapModelNode::OnDisplayNodeAdded(vtkMRMLDisplayNode *dnode)
{
  vtkMRMLSurfaceMapModelDisplayNode* modelDisplayNode =
    vtkMRMLSurfaceMapModelDisplayNode::SafeDownCast(dnode);
  if (modelDisplayNode)
    {
...
//---------------------------------------------------------------------------
void vtkMRMLSurfaceMapModelNode
::SetPolyDataToDisplayNodes()
{
  int ndisp = this->GetNumberOfDisplayNodes();
  for (int n=0; n<ndisp; n++)
    {
    vtkMRMLSurfaceMapModelDisplayNode *dnode = vtkMRMLSurfaceMapModelDisplayNode::SafeDownCast(
...
//---------------------------------------------------------------------------
void vtkMRMLSurfaceMapModelNode
::SetPolyDataToDisplayNode(vtkMRMLSurfaceMapModelDisplayNode* modelDisplayNode)
{
...
//---------------------------------------------------------------------------
bool vtkMRMLSurfaceMapModelNode::GetModifiedSinceRead()
{
